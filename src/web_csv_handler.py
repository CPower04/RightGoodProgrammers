    ...
    if request.method == 'POST':
        ...
        try:
            ...
            meta = {
                ...
                'mem_mb': mem_megabytes(df),
            }
            FRAMES[token] = {'df': df, 'meta': meta, 'skip_bad': skip_bad}

           
            start, end = 0, min(len(df), PAGE_SIZE_DEFAULT)
            table_html = df.iloc[start:end].to_html(classes='table table-striped', index=False, border=0)
            return render_template_string(
                HTML_TEMPLATE,
                meta=meta,
                table=table_html,
